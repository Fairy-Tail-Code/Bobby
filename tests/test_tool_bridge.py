...
@pytest.mark.asyncio
async def test_create_async_tool_func():
...
@pytest.mark.asyncio
async def test_create_async_tool_func_with_multiple_args():
    manager = MagicMock(spec=McpManager)
    manager.call_tool = AsyncMock(return_value="file content")

    tool_func = create_async_tool_func(manager, "workspace", "read_file")
    result = await tool_func(path="/tmp/test.txt", start_line=1, end_line=10)
