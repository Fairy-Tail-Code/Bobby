...
from infrastructure.mcp.tool_bridge import create_async_tool_func


def test_create_async_tool_func():
    ...
    result = asyncio.run(tool_func(cmd="echo hello"))

    assert result == "command output"
    manager.call_tool.assert_called_once_with("shell", "run_short_command", {"cmd": "echo hello"})


def test_create_async_tool_func_with_multiple_args():
    manager = MagicMock(spec=McpManager)
    manager.call_tool = AsyncMock(return_value="file content")

    tool_func = create_async_tool_func(manager, "workspace", "read_file")
    result = asyncio.run(tool_func(path="/tmp/test.txt", start_line=1, end_line=10))
